...
#### USAGE:
####  preprocess.py in the same folder as your files, call python3 preprocess.py [FILE]
#### creates preprocessing from raw bike data ad two error files (list, list and errors) for the unprocesseable ones
...
try:
    ...
    with open('error_list.txt', 'a') as handle:
        handle.write( myfile+"\n"+str(e)+"\n\n\n" )
    with open('file_list.txt', 'a') as handle:
        handle.write( myfile+"\n")
